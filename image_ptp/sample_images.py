...
@torch.no_grad()
def sample_ptp(mixed, batch, seq_len, bos, block_len, device):
    """Block-wise O-PTP sampling: draw u, run one forward, take the argmax.
    ...
    while seq.shape[1] - 1 < seq_len:
        ...
        proposed = completion.logits[:, :span, :].float().argmax(dim=-1)
...
def main():
    ...
    for entry in args.students:
        ...
        torch.manual_seed(args.seed)
        save(sample_ptp(mixed, args.num_images, seq_len, bos, args.block_len, device),
             f"ptp_{name}")
        del mixed, inner
